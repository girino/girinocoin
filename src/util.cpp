...
const char * const BITCOIN_CONF_FILENAME = "girinocoin.conf";
const char * const BITCOIN_PID_FILENAME = "girinocoind.pid";
...
#else
    const char* pszModule = "girinocoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "GirinoCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/GirinoCoin";
#else
    // Unix
    return pathRet / ".girinocoin";
#endif
#endif
}
